...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self,catagory,obscuredPhrase,guessed):
        ...
        print('Catagory: {}'.format(self.catagory))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        self.rand=random.randit(1,10)
        if self.rand>self.difficulty:
            return False
        if self.rand<self.difficulty:
            return True
        
    def getPossibleLetters(self,guessed):#return unguessed letters list
        self.possible=[]
        for letter in LETTERS:
            if letter not in guessed:
                if letter in VOWELS:
                    if VOWEL_COST>=250:
                        self.possible.append(letter)
                    else:
                        continue
                else:
                    self.possible.append(letter)
    ...
    def getMove(self,category,obscuredPhrase,guessed):
        self.getPossibleLetters(guessed)
        if len(self.possible)==0:
            return 'pass'
        else:
            if self.smartCoinFlip() is True:
                for suggestion in reversed(self.SORTED_FREQUENCIES):
                    if suggestion in self.possible:
                        return suggestion
            else:
